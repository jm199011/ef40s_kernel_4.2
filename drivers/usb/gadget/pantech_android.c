...
static ssize_t usb_mdm_control_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  	int ret = 0;
    return ret;
}
...
static ssize_t usb_mdm_control_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	uint32_t value;	
	...
	if(value){
		if(_android_dev->enabled){
			...
			kobject_uevent_env(&_android_dev->dev->kobj, KOBJ_CHANGE,
									mdm_control_disconnected);
		}
